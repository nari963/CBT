// OX퀴즈: 112~121회차 기출문제 기반
window.addEventListener('DOMContentLoaded', function() {
  // ox-box가 있는 페이지에서만 동작
  const oxBox = document.getElementById('ox-box');
  if (!oxBox) return;

  // 용어맞추기 문제 데이터 (term.js의 termData와 동일하게 복사)
  const termData = [
  ...
  // termData를 OX 문제로 변환
  let oxQuestions = termData.map((t, idx) => {
    return {
      question: `${idx+1}. "${t.a}"는 "${t.q}"에 대한 올바른 설명인가요?`,
      answer: 'O', // 정답이 맞으면 O, 틀린 선택지로도 X 문제를 만들 수 있음
      explanation: `정답: ${t.a}`
    };
  });

  let currentIdx = 0;
  let oxStatus = Array(oxQuestions.length).fill('');
  const prevBtn = document.getElementById('ox-prev');
  const nextBtn = document.getElementById('ox-next');
  function showOX() {
    const q = oxQuestions[currentIdx];
    if (!q) return;
    oxBox.innerHTML = `
      <div class="ox-question">${q.question}</div>
      <div class="ox-btns">
        <button class="ox-btn" data-ox="O">O</button>
        <button class="ox-btn" data-ox="X">X</button>
      </div>
      <div class="ox-result"></div>
      <div class="ox-explanation" style="display:none;"></div>
    `;
    // 페이지네이션 표시
    const pageBox = document.getElementById('ox-page');
    if (pageBox) {
      pageBox.textContent = `${currentIdx+1} / ${oxQuestions.length}`;
    }
    // 풀이결과 테이블 표시
    const statusTable = document.getElementById('ox-status-table')?.querySelector('tbody');
    if (statusTable) {
      ...
      setTimeout(() => {
        const table = document.getElementById('ox-status-table');
    ...
    const resultBox = oxBox.querySelector('.ox-result');
    const explanationBox = oxBox.querySelector('.ox-explanation');
    oxBox.querySelectorAll('.ox-btn').forEach(btn => {
      btn.onclick = () => {
        if (btn.getAttribute('data-ox') === q.answer) {
          resultBox.textContent = '정답입니다!';
          resultBox.style.color = '#005bac';
          oxStatus[currentIdx] = 'O';
        } else {
          resultBox.textContent = `오답입니다. 정답: ${q.answer}`;
          resultBox.style.color = 'red';
          oxStatus[currentIdx] = 'X';
        }
        if (q.explanation) {
        ...
        const statusTable = document.getElementById('ox-status-table')?.querySelector('tbody');
        if (statusTable) {
          statusTable.children[currentIdx].children[1].textContent = oxStatus[currentIdx];
        }
      };
    });
    prevBtn.disabled = currentIdx === 0;
    nextBtn.disabled = currentIdx === oxQuestions.length-1;
  }
  ...
  nextBtn.onclick = () => {
    if (currentIdx < oxQuestions.length-1) {
      currentIdx++;
      showOX();
    }
  };
